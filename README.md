...
async def cmd_addvip(message: Message):
    ...
    if len(parts) < 2:
        return await message.answer('Формат: /addvip USER_ID DAYS\nПример: /addvip 123456789 30\nДля бессрочного VIP: /addvip USER_ID lifetime')
    user_id = int(parts[1])
    if len(parts) >= 3 and parts[2].lower() in {'life', 'lifetime', 'forever', 'бессрочно'}:
        await db.add_lifetime_vip(user_id, note=f'admin_lifetime:{message.from_user.id}')
        await message.answer(f'✅ Бессрочный VIP выдан пользователю {user_id}')
        try:
            await bot.send_message(user_id, '✅ VIP доступ активирован бессрочно.')
        except Exception:
            pass
        return
...
async def main():
    ...
    await db.init()
    setup_scheduler(bot)
    print('CryptoVIPBot v2.1.2 started')
